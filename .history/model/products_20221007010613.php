...
function update_products($name, $filename, $price, $detail, $sale, $idlhh)
{
    if($filename!=""){
        $sql="UPDATE `products` SET `name`='$name',`image`='$filename',`price`='$price',`detail`='$detail',`sale`='$sale',`id_commodities`='$idlh' where `id_hh`=".$id;
    }else{
        $sql="UPDATE `products` SET `name`='$name',`price`='$price',`detail`='$detail',`sale`='$sale',`id_commodities`='$idlh' where `id_hh`=".$id;
    }
